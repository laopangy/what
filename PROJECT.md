...
- 力量训练：内置训练模板作为生成初始值；训练详情中的每个动作都提供“替换”入口，可直接跳转到对应编辑卡片，并从按胸、背、肩、腿、手臂、核心、有氧分类的动作库选择哑铃卧推等偏好动作。进入计划编辑后也可新增、修改或删除任意动作，并设置动作名称、部位、组数、目标、休息时间及重量＋次数、仅次数、按时长三种记录方式，支持动作库之外的自定义动作；完成一组后自动启动休息倒计时
...
| 2026-08-30 | Codex | Fitness/动作选择 | 为训练详情的每个动作增加可见的“替换”入口，并在计划编辑器中加入按胸、背、肩、腿、手臂、核心和有氧分类的动作库；选择哑铃卧推等动作时自动带入部位、组数、目标、休息和记录方式，同时保留完全自定义输入 | `Fitness/client/src/App.tsx`, `PROJECT.md` |
