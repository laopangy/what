...
- **统一应用设置**：“账号与服务”集中展示应用名称、版本和模块信息，并管理高德三项密钥（复用加密仓库，不在肌肉大或户外提供录入表单）。
...
- 默认每次启动都显示安装器；全部必需环境完成后才出现“以后直接启动”按钮
...
- `setup.ps1 -ForceShow` 可忽略跳过偏好，重新打开安装器维护依赖
...
- 安装器不提供密钥录入；DeepSeek 和高德配置统一在应用“账号与服务”设置中管理。
- 服务密钥不参与安装检测、启动提醒或“以后直接启动”的就绪判断。
...
- 饮食记录：无论是“鸡胸肉 200克”“米饭 1碗”等简单分量，还是“沙县鸡腿饭 + 一个鸭腿，鸡腿鸭腿都去皮”等套餐、额外加菜和跨食物修饰，均必须由 DeepSeek 拆分食物、判断常见成品份量，并估算热量、蛋白质、碳水和脂肪。可以直接说“我今天中午吃了……”，系统会识别餐次并在 AI 成功返回后记录到今天对应餐次；不写餐次时根据当前时间归类。界面逐项展示 AI 的营养数据与份量假设，也保留只计算不记录及手动调整流程。项目不包含本地食物库或降级规则，AI 未配置、超时或返回异常时直接报错且不保存估算结果
- 饮食历史：饮食页按所选日期汇总与展示记录，支持前后一天、日历、回到今天和全部已有记录日期列表；手动与 AI 补录均使用所选日期，AI 接口不传日期时兼容默认今天。历史汇总仅以当前身体目标作为参考，不伪造历史目标快照；已有记录仍保存在加密仓库，不清理历史。
- 身体数据：基础资料与目标修改后防抖自动保存并实时重算营养目标；减脂时可填写目标体重和希望完成日期，系统显示周期、目标周减重速度、测算所需每日缺口、计划可执行性及因安全限幅产生的预计完成日期。“日常活动量”不可手动选择，系统根据每周计划中的力量/有氧训练日和有时长的恢复活动自动分析，计划增删改或重新生成后同步更新热量目标。今日体重和可选体脂同样自动写入，并以最新晨重同步当前体重。每天称重后立即评估，但至少积累连续 7 天才根据近期均重趋势以每次 100 kcal、最高 ±400 kcal 的幅度调整饮食目标，调整后冷却 7 天，避免单日水分变化造成频繁改计划；评估结果同步展示在身体页和自动生成计划中。展示最近八次体重趋势，并支持直接修改历史记录的日期、体重与体脂或确认后删除
- 目标计算：根据性别、年龄、身高、体重、计划分析出的活动量和增肌/减脂/保持目标，使用 Mifflin-St Jeor 公式依次计算基础代谢、维持热量、计划热量缺口或盈余和建议摄入量；未设置减脂期限时默认从维持热量扣除 350 kcal，设置目标后按体重差和剩余天数估算所需缺口，超过约 0.9 kg/周时限制为最多 1000 kcal/天，目标 BMI 低于 18.5 或已经达到目标时停止继续制造计划缺口。最终目标叠加连续体重趋势修正，不需要用户再次手动扣减。身体数据页展示完整计算链、建议餐次热量分配，今日总览在存在饮食记录时显示已记录摄入与维持热量的阶段差值，并提示全天记录完整后才接近实际缺口；同时计算蛋白质、碳水、脂肪和饮水目标
...
- 五步流程：时间与范围 → 交通与活动 → 目的地与路线 → 过夜与住宿 → 完整行程。支持 1～7 天、具体起止日期/时刻；单程交通时间使用小时＋分钟，可留空，但与距离至少填一项。每天活动预算同样使用小时＋分钟，内部按分钟存储；日期按北京时间解释
- 同行人员：初始为空，通过“添加人员类别”按需添加、填写及移除分组，不预填人数，至少填写 1 人后继续；成年人（不含老人）、老人、儿童分别计数并自动合计，女性人数作为其中可选统计，不重复计数、不按性别推断体力；旧记录兼容读取。数值输入支持清空、聚焦全选与前导零规范化，避免删除后被强制补零
- 当前交通入口优先聚焦自驾和骑行；旧公共交通/铁路行程保留读取兼容，重新规划提示改选。高德请求在进程范围统一排队（至少间隔 1.1 秒），相同进行中请求合并，QPS 超限最多重试两次；10021 明确提示接口请求频率超限，不再误导为密钥错误
- 地点确认：文字或系统语音输入地点后，从高德 POI 搜索结果确认具体位置；家庭地址可设为默认搜索值，不再默认为上海/莫干山
- 目的地推荐：从周边 50km 与所在省份的有限候选池中，以相关性检索最多 25 个周边 POI，交错区域候选后最多校验 24 个地点的真实往返路线。按实际去程路程分档：50km 上限显示 10km 内、10–20、20–30、30–40、40–50km；边界只归入较近一档。超过 100km 时扩大档距避免过多分组；不再找到四个结果即停止。空档仅表示本次有限检索未匹配，不是该范围没有地点
- 公路车与自驾分开：自驾推荐游玩目的地；公路车搜索路口/骑行驿站，突出道路指示、往返里程和总骑行时间，支持往返与当天途经点环线（可能重合）。环线须设总里程上限并选途经点，按真实三段路闭合；往返可选总里程上限。骑行本身计入每日骑行预算，不再额外要求到达景点后的活动折返点或默认游览时长
- 公路车可达性边界：对缺少几何、导航明确提示台阶/非铺装/禁骑的路线不予生成或推荐；路面、爬升、车流、补给与终点通行未获得可靠证据时均标为待核实，不宣称已验证“公路车友好”。多日骑行最终保存前校验完整总里程
...
- 密钥配置：「账号与服务」设置输入 Web 端 Key、JS 安全密钥、Web 服务 Key；不回显已保存表单值，Web 服务 Key 不返回前端。桌面本地版 JS 密钥在浏览器运行时可见，公开部署前需要安全代理。后端只监听 127.0.0.1，限制来源，不记录请求/密钥日志
...
| 2026-08-31 | Codex | Fitness/营养汇总精度 | 今日总览和饮食记录页的热量及三大营养素汇总统一按 1 位小数四舍五入，消除浮点累加产生的 `171.89999999999998` 等显示误差 | `Fitness/client/src/App.tsx`, `PROJECT.md` |
...
| 2026-09-04 | Codex | Outdoor/人员与输入体验 | 新增年龄分组、女性子集统计；修复前导零，交通和活动时长改为小时＋分钟，交通时间/距离至少填一项；聚焦自驾/骑行，增加高德统一限速、请求合并、有限重试与 10021 准确提示 | `Outdoor/client/src/**`, `Outdoor/server/src/**`, `Outdoor/tests/browser-smoke.mjs`, `CLAUDE.md`, `PROJECT.md` |
| 2026-09-04 | Codex | Outdoor/距离分档与公路车 | 按真实去程距离分档推荐，扩大有限候选覆盖并去除四条截断；自驾目的地与公路车过程分离，支持往返/途经点环线、总里程与每日骑行预算，过滤已知不适宜道路提示并明示待核实项；新增分档边界和公路车回归测试 | `Outdoor/**`, `PROJECT.md`, `CLAUDE.md` |
...
| 2026-09-04 | Codex | 应用设置/安装器 | 账号与服务增加应用信息及高德配置，移除户外密钥表单；安装器移除 DeepSeek 录入、检测与启动提醒，保留已有配置 | `Music/client/src/components/dashboard/SettingsPage.tsx`, `MapSettingsCard.tsx`, `Outdoor/client/src/App.tsx`, `Outdoor/server/src/index.ts`, `setup.ps1`, `CLAUDE.md`, `PROJECT.md` |
