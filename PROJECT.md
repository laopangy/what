...
- 目标计算：根据性别、年龄、身高、体重、计划分析出的活动量和增肌/减脂/保持目标，使用 Mifflin-St Jeor 公式依次计算基础代谢、维持热量、计划热量缺口或盈余和建议摄入量；减脂默认从维持热量扣除 350 kcal，并叠加连续体重趋势产生的修正，不需要用户再次手动扣减。身体数据页展示完整计算链，今日总览在存在饮食记录时显示已记录摄入与维持热量的阶段差值，并提示全天记录完整后才接近实际缺口；同时计算蛋白质、碳水、脂肪和饮水目标
