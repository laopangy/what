...
- 饮食记录：输入“鸡胸肉 200克”“米饭 1碗”等简单分量时优先从本地食物库快速换算；输入“沙县鸡腿饭 + 一个鸭腿，鸡腿鸭腿都去皮”等套餐、额外加菜、去皮、少饭或少油描述时，由 AI 理解跨食物修饰、按中国常见成品份量拆分食物，并估算热量、蛋白质、碳水和脂肪。界面逐项展示营养数据、份量假设和 AI/本地来源，结果可在保存前手动调整；AI 不可用时尽量回退本地食物库。也支持营养标签输入，自动完成 kJ → kcal 和实际分量换算
