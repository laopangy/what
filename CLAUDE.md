...
出行表单的交通上限、每天活动预算使用小时＋分钟填写；交通时长可不填，但时长和距离至少一项必填。数值框可清空且不残留前导零。同行人员初始为空，使用“添加人员类别”按需添加或移除分组并手动填写人数；至少填写 1 人后才能继续。人员按成年人（不含老人）、老人、儿童合计，女性为其中的可选统计，不重复计入总人数。当前交通选择显示自驾/骑行，保留旧公共交通记录兼容。
