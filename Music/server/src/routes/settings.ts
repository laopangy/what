...
settingsRouter.post("/ai", (req, res, next) => {
  try {
    ...
    const baseUrl = body.baseUrl || config.ai.baseUrl;
    const model = body.model || config.ai.model;
    ...
    config.ai.apiKey = body.apiKey;
    config.ai.baseUrl = baseUrl;
    config.ai.model = model;
    res.json({ success: true, data: { configured: true, provider: "deepseek", baseUrl, model } });
  } catch (error) { next(error); }
});
