        ...
        {session ? <section className="panel rounded-xl overflow-hidden">
          ...
          <div className="p-5 space-y-5">
            ...
            {session?.exercises.map((exercise, exerciseIndex) => <div key={exercise.id}>
              <div className="flex items-center gap-3 mb-2"><span className="w-7 h-7 rounded bg-panel grid place-items-center text-muted text-[11px]">{exerciseIndex + 1}</span><div className="flex-1"><h3 className="font-semibold">{exercise.name}</h3><p className="text-muted text-[10px]">{exercise.muscle} · 目标 {exercise.reps} · 休息 {exercise.restSeconds}秒{exercise.estimatedDurationMinutes ? ` · 预计 ${exercise.estimatedDurationMinutes}分钟` : ""}</p></div><button type="button" className="btn-quiet !px-2.5 !py-1.5 flex items-center gap-1 text-[10px]" onClick={() => editPlan(session, exercise.id)}><Pencil size={11}/>编辑</button></div>
              <div className="ml-10 space-y-1.5">{Array.from({ length: exercise.sets }, (_, index) => { const key = `${exercise.id}-${index + 1}`; const value = sets[key] ?? { weight: "", reps: "", duration: "", done: false }; const trackingType = exercise.trackingType || "weight_reps"; return <div key={key} className={`grid ${trackingType === "weight_reps" ? "grid-cols-[38px_1fr_1fr_72px]" : "grid-cols-[38px_1fr_72px]"} gap-2 items-center rounded-lg p-2 border ${value.done ? "border-mint/50 bg-mint/10" : "border-border/70 bg-black/10"}`}><span className="text-muted text-center">{index + 1}</span>{trackingType === "weight_reps" && <label className="relative"><input className="field !py-2 pr-8" type="number" min="0" placeholder="重量" value={value.weight} onChange={(event) => updateSet(key, { weight: normalizeNumericInput(event.target.value) })}/><span className="absolute right-2 top-2.5 text-muted text-[10px]">kg</span></label>}{trackingType !== "duration" ? <label className="relative"><input className="field !py-2 pr-8" type="number" min="0" placeholder="次数" value={value.reps} onChange={(event) => updateSet(key, { reps: normalizeNumericInput(event.target.value) })}/><span className="absolute right-2 top-2.5 text-muted text-[10px]">次</span></label> : <label className="relative"><input className="field !py-2 pr-8" type="number" min="1" placeholder="时长" value={value.duration} onChange={(event) => updateSet(key, { duration: normalizeNumericInput(event.target.value) })}/><span className="absolute right-2 top-2.5 text-muted text-[10px]">秒</span></label>}<button className={value.done ? "btn-primary !p-2" : "btn-quiet !p-2"} onClick={() => completeSet(key, exercise.restSeconds)}>{value.done ? <Check size={15} className="mx-auto"/> : "完成"}</button></div>; })}</div>
            </div>)}
            {session.activityType !== "daily" && <div className="grid sm:grid-cols-2 lg:grid-cols-[120px_120px_120px_1fr_auto] gap-3 pt-4 border-t border-border items-end"><label><span className="label">时长（分钟）</span><input className="field" type="number" min="1" value={duration} onChange={(event) => setDuration(normalizeNumericInput(event.target.value))} /></label><label><span className="label">距离 km</span><input className="field" type="number" min="0" step=".1" placeholder="可选" value={distance} onChange={(event) => setDistance(normalizeNumericInput(event.target.value))} /></label><label><span className="label">爬升 m</span><input className="field" type="number" min="0" placeholder="可选" value={elevation} onChange={(event) => setElevation(normalizeNumericInput(event.target.value))} /></label><label><span className="label">运动感受</span><input className="field" placeholder="路线、强度、身体状态……" value={notes} onChange={(event) => setNotes(event.target.value)} /></label><button className="btn-primary h-[38px]" disabled={saving} onClick={finish}>{saving ? "保存中…" : "结束并保存"}</button></div>}
          </div>
  ...
  return <>
    ...
    {open && <div className="basis-full w-full mt-2 rounded-lg border border-border/80 bg-black/15 p-3 text-[10px]">
      {!process ? <p className="text-muted">开始识别后，这里会显示等待时间、估算方式和每项汇总过程。</p> : <div className="space-y-2">
        ...
        {running ? <div className="flex gap-2"><LoaderCircle size={12} className="mt-0.5 shrink-0 text-accent-light animate-spin"/><div><strong>DeepSeek 正在拆分并估算 · {duration.toFixed(1)} 秒</strong><p className="text-muted mt-0.5">正在等待 AI 返回食物分项、份量假设和营养数据；若首轮 JSON 不完整会自动请求一次修复，始终不会使用本地数据替代。</p></div></div> : process.status === "failed" ? <div className="flex gap-2 text-red-300"><X size={12} className="mt-0.5 shrink-0"/><div><strong>AI 计算未完成 · {duration.toFixed(1)} 秒</strong><p className="mt-0.5">{process.error}</p></div></div> : result && <>
          <div className="flex gap-2"><Check size={12} className="mt-0.5 shrink-0 text-accent-light"/><div><strong>AI 已返回分项估算 · {duration.toFixed(1)} 秒</strong>{result.note && <p className="text-muted mt-0.5">{result.note}</p>}</div></div>
          ...
          {process.mode === "record" && <div className="flex gap-2"><Check size={12} className="mt-0.5 shrink-0 text-accent-light"/><strong>已写入 {process.recordedDate} 的饮食记录</strong></div>}
        </>}
      </div>}
    </div>}
  </>;
...
function Nutrition({ data, refresh, notify }: { data: FitnessState; refresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  const meals = data.meals.filter((meal) => meal.date === form.date);
  const recordedDates = [...new Set(data.meals.map((meal) => meal.date))].sort((a, b) => b.localeCompare(a));
  const busy = saving || calculating || recordingFromText;
  const selectDate = (date: string) => { if (date && date <= today() && !busy) setForm((current) => ({ ...current, date })); };
  const moveDate = (offset: number) => { const value = new Date(`${form.date}T12:00:00`); value.setDate(value.getDate() + offset); selectDate(`${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, "0")}-${String(value.getDate()).padStart(2, "0")}`); };
  ...
  const completeNutritionProcess = (mode: NutritionProcessState["mode"], startedAt: number, result: FoodCalculation, recordedDate?: string) => setNutritionProcess({ recordedDate, mode, query: foodQuery.trim(), status: "completed", startedAt, finishedAt: Date.now(), result });
  ...
  const calculate = async () => {
    ...
    try {
      ...
      notify(`AI 已拆分并估算 ${result.items.length} 项食物`);
  ...
  const add = async () => {
    ...
    try { setSaving(true); await api.addMeal({ ...form, calories: Number(form.calories) || 0, protein: Number(form.protein) || 0, carbs: Number(form.carbs) || 0, fat: Number(form.fat) || 0 }); setForm({ ...blank, date: form.date }); await refresh(); notify(`${form.date} 饮食记录已添加`); }
    catch (error) { notify(error instanceof Error ? error.message : "保存失败"); } finally { setSaving(false); }
  };
  const recordFromText = async () => {
    if (busy) return;
    if (!foodQuery.trim()) return notify("例如：中午吃了沙县鸡腿饭，另外加一个去皮鸭腿");
    const startedAt = beginNutritionProcess("record");
    try {
      setRecordingFromText(true);
      const result = await api.addMealFromText(foodQuery, form.date);
      setCalculation(result.calculation);
      await refresh();
      completeNutritionProcess("record", startedAt, result.calculation, result.meal.date);
      setFoodQuery("");
      notify(`已自动记录到 ${result.meal.date} ${mealNames[result.meal.mealType]}`);
  ...
  return (
    <div className="space-y-5">
      <header><p className="text-accent-light text-[10px] tracking-[.2em] uppercase mb-2">Nutrition</p><h1 className="text-2xl font-semibold">饮食记录与历史</h1><p className="text-muted mt-1">先盯住总热量和蛋白质，记录不必追求绝对精确。</p></header>
      <section className="panel rounded-xl p-4 space-y-3"><div className="flex flex-wrap items-end gap-2"><button className="btn-quiet" disabled={busy} onClick={() => moveDate(-1)}>前一天</button><label><span className="label">查看 / 记录日期</span><input aria-label="饮食记录日期" className="field" type="date" max={today()} value={form.date} disabled={busy} onChange={(event) => selectDate(event.target.value)}/></label><button className="btn-quiet" disabled={busy || form.date >= today()} onClick={() => moveDate(1)}>后一天</button><button className="btn-quiet" disabled={busy || form.date === today()} onClick={() => selectDate(today())}>回到今天</button><label><span className="label">已有记录的日期 · {recordedDates.length} 天</span><select aria-label="饮食历史日期" className="field" disabled={busy} value={recordedDates.includes(form.date) ? form.date : ""} onChange={(event) => selectDate(event.target.value)}><option value="">选择历史日期</option>{recordedDates.map((date) => <option key={date} value={date}>{date}</option>)}</select></label></div><p className="text-muted text-[10px]">以下为 {form.date} 的营养汇总；对比值为当前身体目标，不是历史目标快照。手动与 AI 记录均保存到此日期。</p></section>
      ...
      <div className="grid xl:grid-cols-[.8fr_1.2fr] gap-4 items-start">
        <section className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-4"><Plus size={16} className="text-accent-light"/><h2 className="font-semibold">添加一餐</h2></div><div className="rounded-lg border border-accent/30 bg-accent/10 p-3 mb-4"><label><span className="label !text-accent-light flex items-center gap-1"><Sparkles size={12}/>AI 智能饮食记录</span><input className="field" placeholder="例如：中午吃了沙县鸡腿饭，另外加一个去皮鸭腿" value={foodQuery} onChange={(event) => { setFoodQuery(event.target.value); setCalculation(null); setNutritionProcess(null); }} onKeyDown={(event) => { if (event.key === "Enter") recordFromText(); }}/></label><div className="flex flex-wrap gap-2 mt-2"><button className="btn-primary flex-1 min-w-32" disabled={recordingFromText || calculating} onClick={recordFromText}>{recordingFromText ? "AI 识别并记录中…" : "识别并自动记录"}</button><button className="btn-quiet shrink-0" disabled={calculating || recordingFromText} onClick={calculate}>{calculating ? "AI 计算中…" : "只计算不记录"}</button><NutritionProcess process={nutritionProcess} elapsedMs={processElapsedMs} open={processOpen} toggle={() => setProcessOpen((current) => !current)}/></div>{calculation && <div className="mt-3 border-t border-accent/20 pt-2 space-y-1.5"><div className="flex justify-between text-[10px] text-accent-light"><span>AI 分项估算</span><strong>合计 {calculation.calories} kcal</strong></div>{calculation.items.map((item, index) => <div key={`${item.name}-${index}`}><div className="flex justify-between text-[10px]"><span>{item.name} · {item.amount}</span><span className="text-muted">{item.calories} kcal · 蛋白 {item.protein}g · 碳水 {item.carbs}g · 脂肪 {item.fat}g</span></div>{item.note && <p className="text-[9px] text-amber-300/80 mt-0.5">{item.note}</p>}</div>)}{calculation.note && <p className="text-[9px] text-muted border-t border-accent/15 pt-1.5">{calculation.note}</p>}</div>}<p className="text-[9px] text-muted mt-2">先选择上方日期，再说出“早餐 / 中午 / 晚上 / 加餐吃了什么”。以所选日期为准，DeepSeek 识别餐次并入账；AI 未成功返回时不会保存估算结果。结果仍可使用下方表单手动调整。</p></div><div className="grid grid-cols-2 gap-3">
          ...
          {(["calories", "protein", "carbs", "fat"] as const).map((key) => <label key={key}><span className="label">{{ calories: "热量 kcal", protein: "蛋白质 g", carbs: "碳水 g", fat: "脂肪 g" }[key]}</span><input className="field" type="number" min="0" value={form[key]} onChange={(event) => setForm({ ...form, [key]: normalizeNumericInput(event.target.value) })}/></label>)}
        </div><button className="btn-primary w-full mt-4" disabled={saving} onClick={add}>{saving ? "保存中…" : `添加到 ${form.date}`}</button></section>

        <section className="panel rounded-xl overflow-hidden"><div className="px-5 py-4 border-b border-border flex justify-between"><h2 className="font-semibold">{form.date === today() ? "今日饮食" : `${form.date} 饮食`}</h2><span className="text-muted text-[11px]">{meals.length} 条记录</span></div>{meals.length === 0 ? <div className="py-16 text-center text-muted"><Apple size={28} className="mx-auto mb-3 opacity-40"/><p>这一天还没有饮食记录，可在左侧补录。</p></div> : <div className="divide-y divide-border">{meals.map((meal) => <div key={meal.id} className="px-5 py-3 flex items-center gap-3"><span className="w-10 text-muted text-[11px]">{mealNames[meal.mealType]}</span><div className="flex-1"><p>{meal.name} <span className="text-muted text-[11px]">· {meal.amount}</span></p><p className="text-muted text-[10px] mt-1">蛋白 {meal.protein}g · 碳水 {meal.carbs}g · 脂肪 {meal.fat}g</p></div><strong className="text-accent-light">{meal.calories}<small className="text-muted text-[9px] ml-1">kcal</small></strong><button className="text-muted hover:text-red-400 p-2" onClick={() => remove(meal.id)}><Trash2 size={14}/></button></div>)}</div>}</section>
      </div>
    </div>
  );
}
...
const weightUnitSymbols: Record<WeightUnit, string> = { kg: "kg", jin: "斤", lb: "lb" };
...
function BodyData({ data, refresh, notify }: { data: FitnessState; refresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  useEffect(() => {
    const input = { name: profile.name.trim(), sex: profile.sex, age: Number(profile.age), heightCm: Number(profile.heightCm), weightKg: Number(profile.weightKg), goal: profile.goal, targetWeightKg: profile.targetWeightKg, targetDate: profile.targetDate };
    const current = { name: data.profile.name, sex: data.profile.sex, age: data.profile.age, heightCm: data.profile.heightCm, weightKg: data.profile.weightKg, goal: data.profile.goal, targetWeightKg: data.profile.targetWeightKg, targetDate: data.profile.targetDate };
  ...
  const changeWeightUnit = (nextUnit: WeightUnit) => {
    ...
    setEditingWeight((current) => {
      if (!current?.weightKg || !Number.isFinite(Number(current.weightKg))) return current;
      return { ...current, weightKg: String(convertWeight(Number(current.weightKg), weightUnit, nextUnit)) };
    });
  ...
  useEffect(() => {
    ...
    const weightKg = convertWeight(Number(editingWeight.weightKg), weightUnit, "kg"); const fat = editingWeight.bodyFat ? Number(editingWeight.bodyFat) : undefined;
    ...
    return () => window.clearTimeout(id);
  }, [editingWeight, data.weights, weightUnit]);
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-accent-light text-[10px] tracking-[.2em] uppercase mb-2">Body & target</p><h1 className="text-2xl font-semibold">身体数据与目标</h1><p className="text-muted mt-1">所有内容修改后自动保存，热量与营养目标也会实时重算。</p></div><label className="min-w-32"><span className="label">体重单位</span><select className="field !py-2" aria-label="身体数据体重单位" value={weightUnit} onChange={(event) => changeWeightUnit(event.target.value as WeightUnit)}>{weightUnitOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></header>
      <div className="grid xl:grid-cols-[1fr_.9fr] gap-4 items-start">
        <section className="panel rounded-xl p-5"><div className="flex items-center justify-between gap-3 mb-5"><div className="flex items-center gap-2"><Target size={17} className="text-accent-light"/><h2 className="font-semibold">基础资料</h2></div><SaveStatus status={profileStatus}/></div><div className="grid sm:grid-cols-2 gap-3">
          ...
          <label><span className="label">当前体重 {weightUnitSymbols[weightUnit]}</span><input className="field" type="number" step=".1" value={convertWeight(profile.weightKg, "kg", weightUnit)} onChange={(event) => field("weightKg", convertWeight(Number(normalizeNumericInput(event.target.value)), weightUnit, "kg"))}/></label>
          {profile.goal === "lose" && <><label><span className="label">减脂目标体重 {weightUnitSymbols[weightUnit]}</span><input className="field" type="number" step=".1" min={convertWeight(30, "kg", weightUnit)} max={convertWeight(profile.weightKg, "kg", weightUnit)} placeholder="例如 65" value={profile.targetWeightKg === undefined ? "" : convertWeight(profile.targetWeightKg, "kg", weightUnit)} onChange={(event) => field("targetWeightKg", event.target.value ? convertWeight(Number(normalizeNumericInput(event.target.value)), weightUnit, "kg") : undefined)}/></label><label><span className="label">希望完成日期</span><input className="field" type="date" min={tomorrow()} value={profile.targetDate || ""} onChange={(event) => field("targetDate", event.target.value || undefined)}/></label></>}
        ...
        </div>{profile.goal === "lose" && <div className={`mt-4 rounded-lg border p-3 ${data.profile.goalPaceStatus === "too_fast" || data.profile.goalPaceStatus === "below_healthy_range" ? "border-red-300/30 bg-red-300/5" : "border-accent/25 bg-accent/10"}`}>{data.profile.goalPaceStatus === "unset" ? <p className="text-muted text-[10px]">填写目标体重和完成日期后，系统会自动换算减脂时长、每周速度和每日热量缺口。</p> : data.profile.goalPaceStatus === "achieved" ? <p className="text-accent-light text-[10px]">目标体重已经达到，可以调整目标或切换为保持阶段。</p> : <><div className="flex flex-wrap gap-x-5 gap-y-1 text-[10px]"><span>周期约 <strong>{Math.ceil((data.profile.goalDurationDays || 0) / 7)} 周</strong></span><span>每周约 <strong>{data.profile.weeklyWeightChangeTarget} kg</strong></span><span>测算所需缺口 <strong>{data.profile.requiredDailyCalorieDeficit} kcal/天</strong></span></div>{data.profile.goalPaceStatus === "too_fast" && <p className="text-red-300 text-[9px] mt-2">这个期限要求的速度超过约 0.9 kg/周，系统不会继续压低摄入；当前计划按上限生成，预计完成日期约为 {data.profile.projectedGoalDate}。</p>}{data.profile.goalPaceStatus === "below_healthy_range" && <p className="text-red-300 text-[9px] mt-2">该目标体重对应的 BMI 低于 18.5，系统不会把它视为正常减脂目标，建议先咨询医生或营养师。</p>}{data.profile.goalPaceStatus === "on_track" && <p className="text-accent-light text-[9px] mt-2">期限处于渐进减重范围，计划会按该目标生成，并根据晨重趋势逐周微调。</p>}</>}</div>}</section>

        <div className="space-y-4">
          <section className="panel rounded-xl p-5"><h2 className="font-semibold">每日热量计算</h2><p className="text-muted text-[9px] mt-1 mb-4">基础代谢 × 计划分析出的活动系数 = 维持热量，再叠加目标与体重趋势修正。</p><div className="grid grid-cols-2 gap-3">{[["基础代谢", `${data.profile.bmr} kcal`], ["维持热量", `${data.profile.maintenanceCalories} kcal`], [targetGapLabel, `${Math.abs(targetGap)} kcal`], ["建议摄入", `${data.profile.calorieTarget} kcal`]].map(([label, value]) => <div key={label} className="bg-black/15 border border-border rounded-lg p-3"><p className="text-muted text-[10px]">{label}</p><p className="text-lg font-semibold mt-1">{value}</p></div>)}</div><div className="grid grid-cols-3 gap-2 mt-3">{[["蛋白质", `${data.profile.proteinTarget} g`], ["碳水", `${data.profile.carbsTarget} g`], ["脂肪", `${data.profile.fatTarget} g`]].map(([label, value]) => <div key={label} className="rounded-lg border border-border/70 p-2.5"><p className="text-muted text-[9px]">{label}</p><p className="font-semibold mt-1">{value}</p></div>)}</div><div className="mt-4 pt-4 border-t border-border"><p className="text-muted text-[9px] mb-2">建议餐次热量分配 · 可按实际作息前后调整</p><div className="grid grid-cols-4 gap-2">{(["breakfast", "lunch", "dinner", "snack"] as PlannedMealType[]).map((meal) => <div key={meal} className="rounded-lg bg-black/10 p-2 text-center"><p className="text-muted text-[9px]">{mealNames[meal]}</p><strong className="text-[11px]">{data.profile.mealCalorieTargets[meal]}</strong><span className="text-muted text-[8px] ml-0.5">kcal</span></div>)}</div></div>{data.profile.goal === "lose" && <p className="text-accent-light text-[9px] mt-3">当前减脂目标已包含热量缺口；生成一周计划时会直接读取这里的摄入与蛋白质目标，不需要再次扣除。</p>}</section>
      ...
      <section className="panel rounded-xl p-5"><div className="flex justify-between items-center mb-5"><div><h2 className="font-semibold">体重趋势</h2><p className="text-muted text-[9px] mt-1">可编辑日期、体重和体脂，修改后自动保存。</p></div><span className="text-muted text-[11px]">最近 {weights.length} 次记录 · {weightUnitSymbols[weightUnit]}</span></div>{weights.length === 0 ? <p className="text-muted text-center py-8">修改今日测量后，这里会出现趋势</p> : <><div className="h-40 flex items-end gap-3 border-b border-border px-2">{weights.map((item) => { const height = 28 + ((item.weightKg - min) / Math.max(1, max - min)) * 82; return <div key={item.id} className="flex-1 h-full flex flex-col justify-end items-center gap-2"><span className="text-[10px] text-text">{convertWeight(item.weightKg, "kg", weightUnit)}</span><div className="w-full max-w-12 rounded-t bg-gradient-to-t from-sky/45 to-sky" style={{ height }}/><span className="text-[9px] text-muted pb-2">{item.date.slice(5)}</span></div>; })}</div><div className="mt-4 space-y-2">{recentWeights.map((item) => <div key={item.id} className="rounded-lg border border-border/70 bg-black/10 p-3">{editingWeight?.id === item.id ? <div className="grid sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-end"><label><span className="label">日期</span><input className="field" type="date" value={editingWeight.date} onChange={(event) => setEditingWeight({ ...editingWeight, date: event.target.value })}/></label><label><span className="label">体重 {weightUnitSymbols[weightUnit]}</span><input className="field" type="number" step=".1" value={editingWeight.weightKg} onChange={(event) => setEditingWeight({ ...editingWeight, weightKg: normalizeNumericInput(event.target.value) })}/></label><label><span className="label">体脂 %</span><input className="field" type="number" step=".1" value={editingWeight.bodyFat} onChange={(event) => setEditingWeight({ ...editingWeight, bodyFat: normalizeNumericInput(event.target.value) })}/></label><div className="flex items-center gap-2 pb-1"><SaveStatus status={editStatus}/><button className="btn-quiet !p-2" onClick={() => setEditingWeight(null)} aria-label="关闭编辑"><X size={14}/></button></div></div> : <div className="flex items-center gap-3"><div className="flex-1"><strong>{convertWeight(item.weightKg, "kg", weightUnit)} {weightUnitSymbols[weightUnit]}</strong><span className="text-muted text-[10px] ml-3">{item.date}{item.bodyFat ? ` · 体脂 ${item.bodyFat}%` : ""}</span></div><button className="btn-quiet !p-2" onClick={() => { setEditStatus("idle"); setEditingWeight({ id: item.id, date: item.date, weightKg: String(convertWeight(item.weightKg, "kg", weightUnit)), bodyFat: item.bodyFat ? String(item.bodyFat) : "" }); }} aria-label="编辑体重记录"><Pencil size={13}/></button><button className="text-muted hover:text-red-300 p-2 disabled:opacity-50" disabled={Boolean(deletingWeightId)} onClick={() => removeWeight(item)} aria-label="删除体重记录"><Trash2 size={13}/></button></div>}</div>)}</div></>}</section>
    </div>
  );
}
