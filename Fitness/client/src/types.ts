...
export interface FoodCalculation { name: string; amount: string; grams: number; matchedFood: string; calories: number; protein: number; carbs: number; fat: number; items: FoodCalculationItem[]; unmatched: string[]; estimationMethod?: "local" | "ai"; note?: string; }
