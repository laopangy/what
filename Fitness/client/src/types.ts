...
export interface WorkoutSession { id: string; name: string; weekday: number; focus: string; activityType: ActivityType; targetDurationMinutes: number; targetDistanceKm?: number; targetElevationM?: number; wakeTime?: string; sleepTime?: string; breakfast?: string; lunch?: string; dinner?: string; snack?: string; exercises: Exercise[]; custom?: boolean; }
...
export interface FitnessState { profile: Profile; plan: { id: string; name: string; sessions: WorkoutSession[] }; workoutLogs: WorkoutLog[]; meals: MealEntry[]; weights: WeightEntry[]; }
