...
export interface Profile { name: string; sex: "male" | "female"; age: number; heightCm: number; weightKg: number; activityLevel: number; goal: Goal; targetWeightKg?: number; targetDate?: string; bmr: number; maintenanceCalories: number; calorieGapTarget: number; goalDurationDays?: number; weeklyWeightChangeTarget?: number; requiredDailyCalorieDeficit?: number; projectedGoalDate?: string; goalPaceStatus?: "unset" | "on_track" | "too_fast" | "below_healthy_range" | "achieved"; mealCalorieTargets: Record<PlannedMealType, number>; calorieTarget: number; proteinTarget: number; carbsTarget: number; fatTarget: number; waterTarget: number; }
...
export interface FoodCalculation { name: string; amount: string; grams: number; matchedFood: string; calories: number; protein: number; carbs: number; fat: number; items: FoodCalculationItem[]; unmatched: string[]; estimationMethod: "ai"; note?: string; }
